...
type gateFlags struct {
	ifActive       string // skip unless THIS session ran a turn within dur (or one is in flight)
	ifInactive     string // skip if THIS session ran a turn within dur (or one is in flight) — keepalive/reset shape
...
func (g *gateFlags) specs() []struct {
	...
	return []struct {
		...
		{"--if-active", "FOCI_IF_ACTIVE", "if_active", &g.ifActive},
		{"--if-inactive", "FOCI_IF_INACTIVE", "if_inactive", &g.ifInactive},
		{"--if-user-active", "FOCI_IF_USER_ACTIVE", "if_user_active", &g.ifUserActive},
		{"--if-user-inactive", "FOCI_IF_USER_INACTIVE", "if_user_inactive", &g.ifUserInactive},
	}
}
...
func (g *gateFlags) tryParseGateArg(args []string, i int) (consumed bool, next int) {
	for _, s := range g.specs() {
		...
		if strings.HasPrefix(args[i], s.name+"=") {
			*s.dst = args[i][len(s.name)+1:]
			return true, i
		}
	}
	return false, i
...
// applyEnvDefaults fills any unset gate flag from its FOCI_IF_* env var
// (flag > env > empty), mirroring the precedence used for -a/-s/-m.
