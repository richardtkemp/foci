...
func (p *telegramProvider) SetupSharedFacet(params platform.SharedFacetParams) {
	...
	sharedSTT := p.deps.ResolveSTT(p.deps.STTMap, cfg.STT, config.DerefStr(firstACfg.Defaults.Voice.STT), voice.MergeReplacements(cfg.Defaults.Voice.STTReplacements, firstACfg.Defaults.Voice.STTReplacements))
	sharedTTS := p.deps.ResolveTTS(p.deps.TTSMap, cfg.TTS, config.DerefStr(firstACfg.Defaults.Voice.TTS), config.DerefFloat(firstACfg.Defaults.Voice.TTSRate), voice.MergeReplacements(cfg.Defaults.Voice.TTSReplacements, firstACfg.Defaults.Voice.TTSReplacements))
