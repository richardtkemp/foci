...
// TestOnUser_FiresToolEnd proves that a replayed user message carrying a
// tool_result block routes to the current turn's EventHandler.OnToolEnd with
// the tool_use_id, content, and is_error fields populated. This is the only
// place tool results reach the backend from CC, so it's the primary contract
// for tool-use correlation in delegated turns.
func TestOnUser_FiresToolEnd(t *testing.T) {
	t.Parallel()
	...
	var ends []capturedEnd

	b := &Backend{}
	handler := &delegator.EventHandler{
		OnToolEnd: func(id, _, output string, isError bool) {
			ends = append(ends, capturedEnd{id: id, output: output, isError: isError})
		},
	}
	b.beginTurn(handler)

	falseVal := false
	msg := &UserMessageInbound{
		Message: InboundUserPayload{
			Role: "user",
			Content: []ContentBlock{
				{Type: "tool_result", ToolID: "tu_1", Content: json.RawMessage(`"exit 0"`), IsError: &falseVal},
	...
	if len(ends) != 1 {
		t.Fatalf("OnToolEnd calls = %d, want 1", len(ends))
	}
	if ends[0].id != "tu_1" || ends[0].isError {
		t.Errorf("captured = %+v, want {id:tu_1, isError:false}", ends[0])
	}
}

// TestOnUser_SkipsSubagentReplays proves that user messages from sub-agent
// turns (ParentToolUseID != nil) are ignored — sub-agents consume their own
// tool results internally and the parent only sees the Agent call's final
// aggregated result. Without this guard, nested tool_results would bleed up
// into the parent tracker.
func TestOnUser_SkipsSubagentReplays(t *testing.T) {
	t.Parallel()
	...
	msg := &UserMessageInbound{
		...
		Message: InboundUserPayload{
			Content: []ContentBlock{
				{Type: "tool_result", ToolID: "tu_nested", Content: json.RawMessage(`"nested"`)},
			},
		},
	}
	b.OnUser(msg)
	...
	if fired {
		t.Error("OnToolEnd fired for sub-agent replay; should be skipped")
	}
}
