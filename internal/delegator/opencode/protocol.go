...
// kind-specific raw JSON — decoded per Type in permissions.go.
...
// Properties into the matching typed payload below. Session ID is
// extracted by extractSessionID (subscriber.go) for per-session routing.
