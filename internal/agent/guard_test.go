...
func (stubClient) SendMessage(context.Context, *provider.MessageRequest) (*provider.MessageResponse, error) {
	return nil, fmt.Errorf("stub: not implemented")
}
func (stubClient) CountTokens(context.Context, *provider.MessageRequest) (int, error) {
	return 0, nil
}
func (stubClient) IsCachingAvailable() bool { return false }

func TestGuardToolResult_AutoSummary(t *testing.T) {
	// Proves that auto-summarisation is skipped (fallback used) when GroupResolver is nil.
	tmpDir := t.TempDir()
	...
	// When GroupResolver is nil, no summary is attempted (fallback path)
	a := &Agent{
		MaxResultChars:      10,
		ToolResultTempDir:   tmpDir,
		Client:              stubClient{},
		AutoSummarise:       true,
		GroupResolver:       nil, // no resolver → skip summary
	...
	if !strings.Contains(got, "Result too large") {
		t.Error("expected fallback guard message when GroupResolver is nil")
	}
...
func TestGuardToolResult_SkipsSummaryAboveMaxSummaryChars(t *testing.T) {
	...
	a := &Agent{
		...
		Client:              stubClient{},
		AutoSummarise:       true,
		GroupResolver:       config.NewGroupResolver(config.GroupsConfig{Groups: map[string]string{"powerful": "anthropic/claude-haiku-4-5"}}, nil, true),
