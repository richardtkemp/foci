...
// downscaling.
func maybeDownscaleImage(data []byte, mediaType string, maxPixels int) ([]byte, string) {
	if maxPixels <= 0 {
	...
	log.Infof("image", "downscaled %dx%d (%d bytes) to %dx%d (%d bytes)",
		cfg.Width, cfg.Height, len(data), newW, newH, buf.Len())
