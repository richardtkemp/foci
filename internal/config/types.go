...
type NudgeConfig struct {
	...
	NudgeMaxPerTurn                 *int    `toml:"nudge_max_per_turn"                                hot:"turn" desc:"Maximum total nudge reminders injected across a whole turn, summed over every trigger. 0 (default) = unlimited"`
	NudgeExtractionModel            *string `toml:"nudge_extraction_model"                            hot:"turn" desc:"Model used for the one-shot LLM call that extracts nudge rules from character files. Leave unset to use the backend's own cheap-batch default (currently sonnet)"`
...
type ReflectionConfig struct {
	...
	SessionEndEnabled     *bool   `toml:"session_end_enabled"    default:"true" hot:"event" desc:"Run a reflection pass whenever a session is reset or reclaimed, to capture memories and skills before it ends"`                                    // reflect on /reset and reclaim
	SessionEndPrompt      *string `toml:"session_end_prompt"                    hot:"event" desc:"Path to a custom prompt for session-end reflection. Leave unset for the built-in default, or set to none to disable this trigger"`                 // prompt override (nil = embedded, "none" = disabled)
...
type Config struct {
	...
	Evals              EvalsConfig               `toml:"evals"`            // rubric registry: scoring axes for traces, read from files
