...
func TestLookupValueAgentSection(t *testing.T) {
	cfg := &Config{}
	agent := AgentConfig{Loop: AgentLoopConfig{MaxToolLoops: Ptr[int](30)}}

	got := LookupValue(cfg, agent, "agent", "loop.max_tool_loops")
	if got != "30" {
		t.Errorf("LookupValue(agent, loop.max_tool_loops) = %q, want %q", got, "30")
	}
}
