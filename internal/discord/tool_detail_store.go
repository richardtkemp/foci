...
func (s *ToolDetailStore) Store(messageID int64, compact, fullInput, result string) {
	...
	if err != nil {
		log.Warnf("discord_tool_detail_store", "store message_id=%d: %v", messageID, err)
	}
}
...
func (s *ToolDetailStore) LoadAll() (map[int64]toolResultEntry, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &entry.compactText, &entry.fullInput, &entry.result); err != nil {
			log.Warnf("discord_tool_detail_store", "scan row: %v", err)
			continue
		}
		result[id] = entry
	}
...
func (s *ToolDetailStore) ExpireAndVacuum() {
	...
	if err != nil {
		log.Warnf("discord_tool_detail_store", "expire: %v", err)
		return
	}
	n, _ := res.RowsAffected()
	if n > 0 {
		log.Infof("discord_tool_detail_store", "expired %d old tool detail entries", n)
	}

	if _, err := s.db.Exec("PRAGMA incremental_vacuum"); err != nil {
		log.Warnf("discord_tool_detail_store", "incremental_vacuum: %v", err)
	}
}
...
func (s *ToolDetailStore) Count() int {
	...
	if err := s.db.QueryRow("SELECT COUNT(*) FROM tool_call_details").Scan(&n); err != nil {
		log.Warnf("discord_tool_detail_store", "count: %v", err)
	}
	return n
}
