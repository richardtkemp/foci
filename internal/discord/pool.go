...
// Pool manages a set of secondary Discord bots for facet sessions.
type Pool struct {
	bots        []*botEntry
	mu          sync.Mutex
...
func (p *Pool) collectStaleLocked() []reclaimTarget {
	...
	for _, e := range p.bots {
		...
		if lastStr == "n/a" {
			log.Infof("discord", "reclaiming facet bot (session %s not found)", sk)
		...
		if now.Sub(lastTime) > p.sessionTTL {
			log.Infof("discord", "reclaiming facet bot (session %s idle for %v, TTL %v)",
