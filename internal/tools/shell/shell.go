...
func execCommand(ctx context.Context, params json.RawMessage, store *secrets.Store, bwStore *bitwarden.Store, autoBackgroundSecs int, notifier *tools.AsyncNotifier, workDir string, registry *tools.Registry, spillThreshold int64, spillTempDir string, extraEnv []string, defaultTimeoutSec int) (tools.ToolResult, error) {
	...
	if store != nil && store.IsBlockedCommand(p.Command) {
		return tools.ToolResult{}, fmt.Errorf("command references a blocked path")
	}
	...
	if sleepRegexp.MatchString(p.Command) {
		return tools.ToolResult{}, fmt.Errorf("sleep is not allowed via exec — use remind for timed check-ins instead")
	}
	...
	if refs := secrets.FindSecretRefs(cmd); refs != nil {
		for _, ref := range refs {
			if !bitwarden.IsBitwardenRef(ref) && !allSecretRefsInHTTPRequestScope(cmd) {
				return tools.ToolResult{}, fmt.Errorf("{{secret:}} templates are not allowed in exec — use the http_request tool or foci_http_request shell function instead")
			}
		}
	}
	...
	if bwStore != nil {
		...
		if err != nil {
			return tools.ToolResult{}, fmt.Errorf("resolve bitwarden secrets: %w", err)
		}
		cmd = resolved
	}
...
func execDirect(ctx context.Context, cmd, displayCmd string, timeout time.Duration, background bool, store *secrets.Store, bwStore *bitwarden.Store, workDir string, registry *tools.Registry, outputMode string, spillThreshold int64, spillTempDir string, extraEnv []string) (tools.ToolResult, error) {
	...
	if err != nil {
		return tools.ToolResult{}, fmt.Errorf("create stdout pipe: %w", err)
	}
	stderr, err := proc.StderrPipe()
	if err != nil {
		return tools.ToolResult{}, fmt.Errorf("create stderr pipe: %w", err)
	}

	if err := proc.Start(); err != nil {
		return tools.ToolResult{}, fmt.Errorf("start command: %w", err)
	}
	...
	if outputMode == "separated" {
		result := tools.TextResult(formatSeparatedResult(stdoutSpill.String(), stderrSpill.String(), err, store, bwStore))
		applySpillResult(&result, stdoutSpill, stderrSpill, nil)
		return result, nil
	}
	result := tools.TextResult(formatResult(combinedSpill.String(), err, ctx, timeout, displayCmd, store, bwStore))
...
func execWithAutoBackground(ctx context.Context, cmd, displayCmd string, timeout time.Duration, store *secrets.Store, bwStore *bitwarden.Store, thresholdSecs int, notifier *tools.AsyncNotifier, sessionKey, workDir string, registry *tools.Registry, outputMode string, spillThreshold int64, spillTempDir string, extraEnv []string) (tools.ToolResult, error) {
	...
	var bridge *tools.ExecBridge
	if registry != nil {
		bridgeCtx := tools.WithSessionKey(context.Background(), sessionKey)
		var err error
	...
	formatExecResult := func() tools.ToolResult {
		if outputMode == "separated" {
			result := tools.TextResult(formatSeparatedResult(stdoutSpill.String(), stderrSpill.String(), cmdErr, store, bwStore))
			applySpillResult(&result, stdoutSpill, stderrSpill, nil)
			return result
		}
		result := tools.TextResult(formatResult(combinedSpill.String(), cmdErr, cmdCtx, timeout, displayCmd, store, bwStore))
		applySpillResult(&result, nil, nil, combinedSpill)
		return result
	}

	return tools.RunInBackground(ctx, tools.BackgroundParams{
		SessionKey:    sessionKey,
		Notifier:      notifier,
		ThresholdSecs: thresholdSecs,
		Done:          signal,
		SyncResult: func() (tools.ToolResult, error) {
			cmdCancel()
			if bridge != nil {
				bridge.Close()
			}
		...
		PendingResult:  tools.TextResult(fmt.Sprintf("Command still running (exceeded %ds threshold). Results will be delivered when complete.\n$ %s", thresholdSecs, displayCmd)),
		NotifyOnCancel: true,
	})
}
