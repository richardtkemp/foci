...
func TestTmuxWakeRoutesToCorrectAgent(t *testing.T) {
	...
	_, toolA, _, _ := NewTmuxTool(300, 30, tools.NewAsyncNotifier(func(sk, msg, replyTo, trigger string) {
		wakeA.Add(1)
	}), nil, "", false, 30, 0, "")
	_, toolB, _, _ := NewTmuxTool(300, 30, tools.NewAsyncNotifier(func(sk, msg, replyTo, trigger string) {
		wakeB.Add(1)
	}), nil, "", false, 30, 0, "")
