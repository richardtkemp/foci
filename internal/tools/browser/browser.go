...
func (m *BrowserManager) Timeout() time.Duration {
	return tools.ResolveTimeout(m.config.TimeoutSec, tools.TimeoutConfig{DefaultSec: 30})
}
...
func executeBrowserTool(ctx context.Context, params json.RawMessage, mgr *BrowserManager) (tools.ToolResult, error) {
	...
	default:
		return tools.ToolResult{Text: fmt.Sprintf("Unknown action: %q", p.Action)}, nil
	}
}
