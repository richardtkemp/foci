...
func displayFieldValue(sessionKey string, cc CommandContext, key string) (override, effective string) {
	...
	case "show_tool_calls":
		...
		} else if cc.Config.Defaults.ShowToolCalls != nil {
			effective = string(*cc.Config.Defaults.ShowToolCalls)
		}
		return "", effective
	case "show_thinking":
		...
		} else if cc.Config.Defaults.ShowThinking != nil {
			effective = string(*cc.Config.Defaults.ShowThinking)
		}
		return "", effective
	case "stream_output":
		...
		effective = "off"
		if cc.Config.Defaults.StreamOutput {
			effective = "on"
		}
		return "", effective
	case "display_width":
