...
- **Steering** — mid-turn user messages are dispatched directly via `Backend.Inject(SourceSteer)`, which internally chains `Interrupt` (aborts the in-flight turn) followed by `SendUser` (queues the new text as the next user message). The agent's per-session `Inbox.Enqueue` (Phase 6 — TODO #739) handles the routing; the legacy AppendSteer buffer is bypassed for CC backends.
