...
- **Steering** — mid-turn user messages are dispatched directly via `Backend.Interrupt` followed by `Backend.SendCommand`, which aborts the in-flight turn and queues the new text as the next user message. The `MessageQueue.dispatchUrgent` callback (wired at agent setup) handles the routing; the AppendSteer buffer is bypassed for CC backends.
