...
| Memory capture | `compaction.memoryFlush` (pre-compaction) | `[memory_formation]` | Foci adds interval-based and session-end capture. Session-end runs on a branch so it doesn't block the caller. |
...
- **Interval formation** (`[memory_formation] interval_enabled`) — Periodic capture during the session, not just at compaction.
- **Session-end formation** (`session_end_enabled`) — Fires on `/reset` and facet reclaim. Runs on an async branch so it doesn't block the caller.
