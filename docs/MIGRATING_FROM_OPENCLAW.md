...
OpenClaw doesn't have session branching. Foci has two branch types:
- **Facet** (`/facet`) — parallel conversation on a separate bot (or a real transcript fork on delegated backends), sharing the parent's cache prefix.
- **Spawn** (`spawn` tool) — sub-call with four context modes for different autonomy levels: `raw` (prompt only, no tools), `character` (identity only), `clone` (full async branch with tools and the parent's conversation context), and `explore` (read-only, Haiku).
...
- **Multi-platform support.** Telegram and Discord are both fully supported messaging platforms. The platform abstraction is clean and PRs for others are welcome.
...
- **Native apps.** No macOS or iOS clients. Android app is released.
...
| Memory capture | `compaction.memoryFlush` (pre-compaction) | `[reflection]` + `[maintenance]` | Foci adds interval-based and session-end reflection (memory + autogenerated skill formation). Consolidation (`[maintenance]`) curates MEMORY.md periodically. Session-end runs on a branch so it doesn't block the caller. |
...
OpenClaw uses `compaction.memoryFlush` — a pre-compaction agentic turn that prompts the agent to store durable memories before the session is summarized. Foci separates this into three independent mechanisms:
...
- **Session-end reflection** (`[reflection] session_end_enabled`) — Fires on `/reset` and facet reclaim. Runs on an async branch so it doesn't block the caller.
- **Consolidation** (`[maintenance] consolidation_enabled`) — Periodic curation of MEMORY.md from daily files with a configurable size target. Keys: `consolidation_enabled`, `consolidation_time`, etc.
...
| Feature | Status |
|---------|--------|
| WhatsApp, Slack, Teams | Telegram and Discord are fully supported. Others are coming. PRs welcome — the platform abstraction is clean. |
| Native macOS/iOS apps | No plans. Android app is released. |
