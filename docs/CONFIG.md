...
| `user_data_dir` | string | `""` | Chrome user data directory. Empty uses a temp profile. Only used when incognito mode is off (controlled at runtime via the browser tool's `incognito` parameter). Each session has its own browser, and only one of them at a time can open this profile. |
...
| `prompt_ttl` | duration | `3h` | How long an unanswered interactive prompt (permission request, AskUserQuestion) stays live. On expiry it is resolved as a denial/cancel — so a turn blocked waiting on it doesn't orphan — and its message is edited to show it expired. **Effective value is `min(prompt_ttl, 3h)`** — it starts at the delegated-backend `idle_timeout` default (`agent.DefaultIdleTimeout`) and a configured value only ever *lowers* it, so setting anything above 3h has no effect. A prompt must never outlive the backend waiting on it; the idle reaper clears the prompt anyway when it closes that backend. |
...
| `summary_model` | string | `""` | Delegated agents only: model for `foci_summary` and the `/prompts diff` summary, named as the agent's backend expects it (`haiku` for Claude Code, `provider/model` for opencode). Unset = the backend's own cheap model (Claude Code: `haiku`), or the agent's own model on backends without one (opencode, codex). API agents use the `cheap` model group instead. Global: `[tools]`. |
...
| `max_file_read_bytes` | int | `52428800` | Max file size in bytes the `read`/`edit` tools will load. Files larger than this are refused by stat size before being read into memory (large-file OOM guard); use `offset`/`limit` for big files. Global: `[tools]`, per-agent: `[[agents]].tools`. |
| `http_max_spill_bytes` | int | `52428800` | Ceiling on `http_request` response bytes retained (default 50MB). Responses past the inline preview (`max_response_bytes`) spill the full body to disk so it isn't discarded — the agent reads it via the tool-result guard (API path) or by streaming it through `foci-call` (delegated path). This ceiling bounds disk use for untrusted remote bodies; bodies beyond it are truncated on disk (logged). Global: `[tools]`, per-agent: `[[agents]].tools`. |
...
The periodic reflection pass captures both factual memory (into daily memory files) and procedural knowledge (autogenerated skills under `workspace/skills/`) from recent session activity. All sub-features default to enabled. (MEMORY.md consolidation moved to `[maintenance]` — see below.)
