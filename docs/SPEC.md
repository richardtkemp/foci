...
"User activity" means messages from allowed users via the primary platform (Telegram or Discord). It explicitly excludes: CLI-injected messages (`foci send` / `foci branch`), async notifications, agent-to-agent messages, and system-injected messages. The timestamp is stored per session in `session_index.last_user_activity_at`; the agent-level value is the max over that agent's sessions (`SessionIndex.LastUserActivityForAgent`, read via `Runner.LastUserActivity()`). Use these for nudges that should only fire when the user is engaged (or specifically away).
