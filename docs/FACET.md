...
All messages from a facet session route through the correct bot — tool outputs, async notifications, spawn results, and `send_message_to_user` calls all go to the facet bot's chat, not the primary bot.
