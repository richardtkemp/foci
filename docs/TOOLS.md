...
| `todo` | Per-agent task list — add, list, complete, remove, search. SQLite backend with priority ordering (high/medium/low). Tag support for filtering. Items tagged `background` are automatically picked up and worked on in background branch sessions when the user is idle and mana is available — see [HEARTBEAT.md](HEARTBEAT.md). |
