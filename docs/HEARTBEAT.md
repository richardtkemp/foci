...
**Warming the app's open chats.** Set `[keepalive] warm_open_app_chats = true` (default false, global or per-agent) to instead warm **every chat the Android app currently has open** (its pager tabs). The app reports its open-set to the server via a `conversation.openSet` frame on change (there was also an `open` flag on each `hello` resume point; it only ever seeded a per-connection set nothing read (#1742) and was dropped from the wire in #1737 — the PERSISTED set `conversation.openSet` maintains is what `OpenSessionsForAgent` resolves against, and it is correct while the app is backgrounded, which the live view was not), and keepalive fires one warming branch per open session (deduped, in-flight-filtered). When no chats are open it falls back to the default session. Cost scales with the number of open chats (N warming calls per interval), so it is opt-in.
