...
→ shellenv.Apply(cfg.ShellEnvFile)                       ← internal/shellenv; loads the operator's shell rc/env
                                                            ...
                                                            THIS REPLACES PATH. os.Setenv does not update
                                                            /proc/<pid>/environ, so that file keeps showing the
                                                            unit's PATH forever (#1900 — do not audit with it).
→ cfg.DropSubstitutableAutoApproveRules(execguard.Live()) ← must sit immediately after shellenv.Apply, never in
                                                            config.Load: it resolves auto_approve commands
                                                            against the process PATH, which is only final here
