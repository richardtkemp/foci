...
main
 ...
 ├── delegator     → clock, log, modelinfo (Delegator interface, registry, StartOptions, SessionEvents/TurnEvents)
  ...
  │   ├── delegator/pretool    (no foci deps; stdlib + mvdan.cc/sh parser — PreToolUse deny-rule engine, #2028/#2033; shared by config, ccstream, cmd/foci-cc-hook and cmd/foci `pretool`)
...
A **batch** is a one-shot prompt whose answer goes back to an in-process caller and never to a chat: memory consolidation (`periodic/consolidation.go`), nudge extraction (`nudge.Extractor.ExtractViaBatch`), and the delegated `foci_summary` tool (`tools.BatchSummariser`). Every caller passes a `delegator.BatchRequest` with a **`Purpose`** (`delegator.BatchPurpose*`: `consolidation`, `nudge_extraction`, `summary`) and the owning session (`OwnerSessionKey`).
...
- **Launch** (`getOrCreate`, from the `batchSpec` RunBatch registers for the key): `SystemPrompt` = the caller's (replacing the agent's composed prompt; `SystemPromptFunc`/`ModelFunc`/`EffortFunc` are cleared so no backend re-resolves the agent's — opencode's Start calls `SystemPromptFunc` itself), model = the request's, else the backend's `delegator.BatchModelDefaulter` (ccstream: `sonnet`), else the agent's; no launch effort; `StartOptions.SkipPermissions` (CC launches with `--dangerously-skip-permissions`: a batch has no user to answer a prompt, and a prompt would land in a chat). The system prompt reaches CC through the stream-json `initialize` request, not argv, so there is no `MAX_ARG_STRLEN` cliff (#1963).
...
Why this was once separate: the original `RunOnce` (821ff2ca) fled an interactive session that "leaked output to platform and returned ''". The sink/callback handling above is what makes the turn path safe for it now.
...
3. **API log — SQLite** (`api.db`): Same data as JSONL but in a `api_calls` table with indexes on `ts` and `session`. Includes `call_type` column (conversation, compaction, summary, spawn, subagent_turn) and `turn_id`/`agent_id` (see "Turn identity" below).
   ...
   **`purpose` (#1962).** Set only on the rows of a BATCH turn — consolidation,
   nudge extraction, the delegated `foci_summary` tool (`delegator.BatchPurpose*`) —
