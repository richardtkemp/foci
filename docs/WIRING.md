...
main
 ...
 ├── delegator     (no deps — Delegator interface, registry, StartOptions, SessionEvents/TurnEvents)
  ...
  │   └── delegator/opencode   → delegator, log, timeutil (HTTP/SSE OpenCode; registers "opencode" via init())
...
**Rate/session-limit detection → periodic suppression (#1211).** When CC hits a rate / session / usage limit it does *not* emit a structured `rate_limit_event` (we have never observed one — `OnRateLimit` now logs at WARN if it ever fires, so we learn it does). The limit surfaces instead as a **synthetic result**: a no-API-call assistant message (`model=<synthetic>`, `cost=$0`) like `"You've hit your session limit · resets 10:30pm (Europe/London)"`. `looksLikeRateLimit` (`ccstream/ratelimit.go`) matches this in `OnResult` (keyed on the `…limit…reset…` phrasing, not the `<synthetic>` model — that sentinel is used for many benign no-ops) and fires `onRateLimited(detail)`, wired via `Backend.SetOnRateLimited` in `agents_delegated.go`. The agent handler parses the reset clock (`agent.ParseRateLimitReset`, clamped to ≤6h, 1h fallback) and calls `Agent.MarkRateLimited(until)`, which closes the agent's rate-limit gate(s); the gate auto-reopens when `time.Now` passes `until`. User turns are unaffected — they don't consult the gate on the delegated path (`DelegatedTransport.RateLimitGate` is a no-op). This breaks the loop where periodic jobs fired blind into a live cap, each burning a synthetic no-op + warning.
...
- **Usage-limit cancellation:** OpenCode reports rejected account limits as `session.status` retry events rather than `session.error`. The backend distinguishes usage/rate-limit messages from transient retries, trusts reset timestamps only when they carry an explicit UTC offset, and otherwise uses the existing one-hour fallback (Z.AI currently sends a timezone-less wall clock). It engages the agent's existing rate-limit gate/notification hooks, synchronously POSTs `/abort`, and completes the waiting turn. Aborting before completion prevents a replacement turn from consuming delayed abort events from the limited turn.
...
- **Server-death respawn:** When the shared subprocess dies unexpectedly (SIGTERM, subscriber EOF), `finalizeExit` clears the Server's `running` flag, evicts it from the pool, and synthesizes `session.error` for each registered Backend. `Backend.IsRunning()` now checks `running && srv != nil && srv.isAlive()` — previously it returned only `b.running`, which `finalizeExit` couldn't reach, so `getOrCreate` handed back the stale Backend forever. Now the next turn detects the dead server, saves the resume ID, closes the corpse, and `acquireServer` spawns a fresh subprocess (re-ensuring both plugins per `Start`).
...
**Client cache-warmth indicator (#1217).** The app's `cacheExpiry` frame is now driven by the same `last_cache_touch`, not by turn-complete. `Agent.CacheExpiry` returns `last_cache_touch + TTL` (cold/zero when there's no touch), and `Agent.emitCacheExpiry` pushes it via the `onCacheExpiry` hook (wired to `app.SetCacheExpiry` → `activeHub.bindingForSession(...).setCacheExpiry`, deduped) on **every** touch write: `recordTurnActivity` (turn entry), `TouchRootCacheForBranch` (branch warms root), and `ClearCacheTouch` (reset → cold). Previously the frame was emitted only by the app sink at `TurnComplete`, so a session kept warm by keepalive **forks** (which complete off-root) or by external/cron turns (no live app sink) advanced `last_cache_touch` but never refreshed the client — the indicator went stale and showed "expired" while the cache was actually warm. The persistence-hook path fires regardless of whether a live app socket is attached (the durable binding's `cacheExpiryMs` seeds the roster snapshot on reconnect).
