...
**Model fallback** (`[models.fallbacks]`): After retry exhaustion, if the error is fallback-eligible (529, 5xx, `context.DeadlineExceeded`) and a `FallbackResolver` is configured, the agent walks the fallback chain trying alternate models (up to `MaxFallbackDepth=3` hops). Each hop resolves the fallback model's endpoint/format via `ClientProvider.GetClient` and retries `provider.Send`. On success, the response is used for this turn; subsequent tool-loop iterations rebuild with the primary model (fallback is per-request, not sticky). Not triggered by 401, 400, or 429. Configured via `[models.fallbacks]` (global) and `model_fallbacks` (per-agent override).
