...
`RunBatch` (`ccstream/batch.go`, behind RunOnce → nudge extraction and memory consolidation, #1963) writes a non-empty `req.SystemPrompt` to a private temp file under `tempdir.Dir()` and passes it via `--system-prompt-file`, never as a literal `--system-prompt` argv element — matching cctmux's `Start` (below). A composed character system prompt is caller-controlled and unbounded (coach's alone is 131,575 chars), while Linux caps a single execve() argv element at `MAX_ARG_STRLEN` (131072 bytes, not raised by ulimit); crossing it as an argv element fails fork/exec with E2BIG before claude starts, and retries can't help since every attempt hits the same cliff.
