...
**`/android` — native Android onboarding wizard (`command/android_onboard.go`):** `AndroidCommand()` + `androidWizard` walk the user through pairing a device. `Execute` branches on state: app provider disabled → offer to enable (appends `[[platforms]] id="app"` to foci.toml via `appendToFile` and generates `app.api_key` with `secrets.GeneratePassphrase(5)`); enabled + auto-generated key → offer to reveal the key in chat or point at `secrets.toml`; enabled + user-set key → skip to host. Auto-generated detection uses `secrets.IsGeneratedPassphrase` (all-EFF-wordlist hyphenated tokens — no stored marker). The final step emits a `foci://pair?host=…&key=…` string the Android client's `parseQr` accepts. Reads the key via `SecretsStore.Get` (added to the interface); registry handle comes from `cc.AndroidDeps.Registry`. No agent LLM involved — a pure Go wizard like the others.
