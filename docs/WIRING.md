...
**Binding restore across restart + archive (`framestore.go`, `StartAll`, `handleConversationArchive`):** bindings (`h.convs`/`h.bySession`) are in-memory, created only on client frames — so a foci restart empties them and an unsolicited send (`SendToSession`) would `return nil` (drop) until the app reconnects. To close that window, `Hub.StartAll` rebuilds bindings at startup from the durable store: `frameStore.RestorableConvs()` returns every conv with a **visible** frame and a known `agent_id` (a column added to `app_frames`; written by `convBinding.send`), and `ensureBinding(nil, agentID, convID)` recreates each socketless binding (`attach(nil)` is a no-op; seq seeded from `MaxSeq`). **Archive is delete-on-archive, no flag:** the `conversation.archive` frame → `handleConversationArchive` *purges* the conv's frames (`PurgeConv`) — so presence of frames IS the "restore me / non-archived" signal — drops the live binding, flips the session `status` to `archived` (which also removes it from periodic reflection, whose query filters `status='active'`), and fires one **final** reflection if due via `h.reflectOnArchive` (wired in `cmd/foci-gw/main.go` to the owning agent's `Runner.ReflectSessionIfDue`, gated by `SessionIndex.SessionNeedsReflection` — "activity since last reflection"). One-directional: no server-side unarchive (the app's unarchive is a local re-show; the binding is recreated lazily on next use).
