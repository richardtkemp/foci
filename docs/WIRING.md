...
Non-interactive backend execution for headless tasks. `RunOnce` and `RunBatch` carry the owning `SessionKey` explicitly (derived from the turn context). The manager allocates a distinct background-task foci session key for each batch — a child of the owner's ROOT chat (`<agent>/<c|i><id>/b<ts>`), because `ParseSessionKey` only accepts `c`/`i` as the second segment's type rune and an unparseable key indexes with `chat_id=0`/`is_root=0`, unreachable by any family query. It is recorded in `session_index` **only on the Codex multiplexing path** (the owner session has a live `*codex.Backend`), and only when the parent is real and distinct. This is deliberate: the row is unreclaimable by construction — `PruneOrphans` skips empty `file_path` ("backend session, not an orphan") and a self-parented row makes `ArchiveSweep`'s active-branch guard match it against *itself* — so indexing it for every backend made ccstream agents accumulate permanent rows, one per nudge-extraction/consolidation one-shot, while `ArchiveSweep` runs a full index query per candidate. Non-Codex batches run on a fresh backend and are not indexed. `ArchiveSweep` has a third skip alongside current-chat and active-branch: an entry whose `file_path` stats as a **non-regular** file (a directory) is skipped and counted, never handed to `gzipFile` (#1555). Without it a failed gzip `continue`s past `UpdateStatus`, so the row stays `active`, stays a candidate, and re-warns every sweep forever — 544 WARNs in the archived logs from one row (`codex/c7508215090037080023`, `file_path` = `/home/foci/.codex/sessions/`). The skip stats only; an **empty** `file_path` deliberately still falls through to `gzipFile`, which swallows ENOENT and marks it archived, matching `PruneOrphans`' "backend session, not an orphan" rule above.

**A Codex batch runs on its OWN facade, never the owner's (#1585).** `RunBatch` always constructs a fresh `*codex.Backend` via `m.NewBackend()`, with `req.AgentID` set to the owner's AgentID; the session-index check above only decides whether to Upsert, never which object runs the batch. That fresh facade's own `RunBatch` (`codex/batch.go`) calls `Start` with `BatchOnly: true`: since `opts.AgentID` matches the owner's already-running app-server, `Start`'s pool-attach path (keyed on AgentID) multiplexes the new facade onto that SAME shared connection without spawning a second process, and `BatchOnly` skips creating a durable interactive thread — the facade exists only to drive one ephemeral `thread/start {ephemeral:true}` + turn. Codex routes notifications by Codex thread ID: a notification whose thread ID resolves to a batch run is consumed by that run **whatever its method** — the thread lookup is the entire ownership decision. A method whitelist there leaks, because an unlisted event would fall through to whatever backend object dispatch() lands on when the thread isn't registered (streaming the batch's reasoning into a chat, overwriting token usage, renaming a chat from `thread/name/updated`). Because the batch is a distinct Go object, that fallback can no longer reach the owner's own turn/callback state even when it *is* hit.

Cancelling or timing out a batch call does NOT stop its turn server-side, so the thread mapping and pool ref are **not** released the instant `RunBatch` returns on that path — doing so eagerly is exactly what let a late `turn/completed` for the still-live turn fall through to the wrong backend and silently complete an unrelated in-flight turn (#1570). Instead, foci first sends `turn/interrupt` for the batch's own thread id (not via `Interrupt()`, which resolves the caller's interactive session key and would target the wrong thread), and cleanup (drop the thread's bookkeeping, release the pool ref via `Close`) is deferred to a background goroutine that waits for the turn's real end (or the process dying) before running. The interrupt is what makes that wait terminate promptly: without it the orphaned turn keeps running and spending tokens with nothing tracking it, and the waiter blocks indefinitely holding the facade's pool ref, so the agent's app-server is never reaped — and the process-death bound does not help in exactly the case that matters, where sibling sessions keep it alive. `RunBatch` itself still returns synchronously with the batch's text (or the context error on cancel); only the teardown is decoupled from the caller. Codex no longer launches `codex exec --ephemeral`.
...
Non-Codex backends retain their existing fresh-backend `RunBatch` implementations. There is no cross-vendor fallback: a backend that does not implement `BatchRunner` returns an error. `RunOnce` is used by nudge extraction, consolidation, onboarding, and the delegated `foci_summary` tool. `ForkRequest` also carries the owning foci session key so Codex resolves fork targets without a current-thread default.
...
1df68081 fixed this for opencode: codex deletes a thread by `thread/delete` RPC to a live
...
opencode implements it by acquiring — and if the agent is idle, **spawning** — its shared pooled server, the same `acquireServerFn` path `RunBatch` uses. Without that, cleanup was permanently impossible for exactly the agents whose sessions were expiring: an agent idle long enough to have expired sessions has no server *because* it is idle, so every pass logged a per-session WARN and deleted nothing (#1707). The scope is opened only when the sweep has something to delete, so a quiet day spawns nothing. Three sites now share the acquire-spawn-release shape on an unstarted backend — `Backend.Start`, `RunBatch`, `OpenCleanupScope`; `pooledServer` (never spawns) remains correct only for `ForkSession`, which runs mid-turn when the server is up by construction.
...
`RunBatch` (`ccstream/batch.go`, behind RunOnce → nudge extraction and memory consolidation, #1963) never passes a non-empty `req.SystemPrompt` as a literal `--system-prompt` argv element. A composed character system prompt is caller-controlled and unbounded (coach's alone is 131,575 chars), while Linux caps a single execve() argv element at `MAX_ARG_STRLEN` (131072 bytes, not raised by ulimit); crossing it as an argv element fails fork/exec with E2BIG before claude starts, and retries can't help since every attempt hits the same cliff.

Preferred delivery (Dick, 2026-09-23) is a Linux memfd — `createMemfdSystemPrompt` (`memfd_linux.go`) writes the prompt into an anonymous, in-memory `memfd_create(2)` file (no filesystem entry, never touches disk), seeks it to 0, and RunBatch passes it to the child via `cmd.ExtraFiles` + `--system-prompt-file /dev/fd/<3+i>` (i = the file's actual `ExtraFiles` index, not a hardcoded 3). Chosen over a named pipe/fifo (the other disk-free option) because a fifo's open-for-write blocks until a reader opens it — if claude fails to start or dies early, foci's writer hangs — and a fifo is still a filesystem entry a crash can leave behind; the memfd is fully written before exec, so neither failure mode is reachable. `createMemfdSystemPromptFn` is a package-level indirection so tests can force the fallback deterministically. On memfd failure (non-Linux build, or the syscall erroring) RunBatch falls back to `writeSystemPromptFile` (`memfd_other.go` implements the Linux-only function as an error on other platforms), which writes a private temp file under `tempdir.Dir()` and passes it the same way — matching cctmux's `Start` (below), which still uses the on-disk file form since a persistent tmux session has no in-memory prompt string lying around to hand off the way a one-shot batch does.
...
Callback data format: `im:<promptID>:<buttonIndex>`. Prompt IDs are atomic uint64 counters. Callbacks are stored in a global `sync.Mutex`-protected map and auto-expire after the effective prompt TTL (`min(prompt_ttl, idle timeout)`, swept hourly by `setupInteractiveCleanup` → `CleanupExpiredInteractive`). The same tick also calls `app.ExpireBatchPrompts`, which expires the app hub's batched ask forms (`batchPrompts`, outside imStore) on the same TTL, resolving each as the form's own Cancel (#1895). Callbacks are one-shot — removed after handling.
...
`FallbackTTS` deliberately contains **no retry, backoff or wait**: a spoken reply is live, so a provider that says "try again in 4h" is skipped, never waited out — waiting spends quota on audio nobody is still listening for. The only response to a failure is the next provider, or text-only. Its error joins every link's failure with `errors.Join`, keeping a `*ratelimit.Error` reachable via `errors.As` so `logTTSError` still classifies quota exhaustion at info rather than warn.
...
**Protocol:** Line-delimited JSON frames. `ask` (question with options), `answer` (selected option), `cancel` (withdraw), `notify` (completion echo for a previously-answered ask — rendered by editing that ask's chat message in place, standalone message as fallback; see `docs/ASKGW-PROTOCOL.md`), `ack`/`error`. IDs are validated to not contain `:` (platform splits button data on first `:`). Composite keying `(connID, askID)` isolates answers per connection. Message IDs namespaced as `askgw-<askID>-q<idx>` so they never collide with CC permission prompts.
