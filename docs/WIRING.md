...
**Mid-turn injection:** The unified entry point is `Backend.ImmediateInject(ctx, Inject{Source: ..., Text: ...})`. A mid-turn **steer** (`SourceSteer`) is written to CC's stdin at priority `"now"` (`sendUserMessagePriority` → `writer.SendUserPriority`); an in-flight `SourceUser` follow-up goes in at default priority `"next"`. `SourceSystem` never goes in mid-turn at all — it begins a turn atomically at idle (`tryBeginTurn`) or returns `ErrTurnInFlight` for the caller to wait and retry (see "System Injections Never Steer"). Depending on where CC's run loop is, the message either folds into the current ask (mid-tool arrival) or — for `"now"` arriving mid-stream — aborts the ask and is answered in a fresh ask cycle of the same run (claude-code `print.ts` `abort('interrupt')`). Either way it stays inside the current run, so the reply belongs to the current foci turn and no per-inject bookkeeping is needed — see **Idle-keyed turn completion** below. The foci-specific `priority` field is therefore live, not removed. The `interrupt` control request (`Backend.Interrupt`, mirrors the Agent SDK's `client.interrupt()`) is wired to `/stop` **only** — it is *not* used by steer.
