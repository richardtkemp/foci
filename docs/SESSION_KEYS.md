...
The old format used colons as separators:
- `agent:AGENTID:TYPE[:ID][:branch:BRANCHID]`

Migration script: `cmd/migrate-sessions/main.go`

### Old → New Examples

| Old | New |
|-----|-----|
| `agent:main:chat:123` | `main/c123/1709590000` |
| `agent:main:spawn:spawn-1772619494` | `main/i1772619494/1772619494` |
| `agent:main:multiball:mb-1709596800` | `main/c123/1709590000/b1709596800` |
| `agent:main:chat:123:branch:session-end-456` | `main/c123/1709590000/b456` |

### Migration Steps

```bash
# Build migration tool
go build -o migrate-sessions cmd/migrate-sessions/main.go

# Dry run (recommended)
./migrate-sessions \
  -sessions ./sessions \
  -db ./data/session_index.db \
  -dry-run
...
**Important:** Backup sessions and database before migrating!

## Benefits of New Format

1. **Explicit versioning:** Version directories make compaction history clear
2. **Stable chat IDs:** Chat ID remains constant across compactions
3. **Clear parent relationships:** Children grouped with their version
4. **Simpler path mapping:** Key structure matches directory structure
5. **Future-proof:** Easy to add new child types or metadata
