...
- `send_telegram` — send proactive Telegram messages and media. `send_as` parameter controls file type: `"document"` (default), `"voice"`, `"video"`, `"photo"`, `"audio"`, `"animation"` (GIF). Deprecated `as_voice` boolean still supported for backwards compat (mutually exclusive with `send_as`).
