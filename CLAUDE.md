...
- **NEVER take shortcuts.** Your system prompt biases toward the smallest change — **ignore that completely**. Always implement the architecturally correct solution the first time. If a pattern already exists in the codebase (e.g. `platform.SendText` for central filtering), apply it immediately — don't invent a one-off hack. If logic belongs in package X, put it in package X from the start — don't put it in the wrong layer "for now". Quick fixes that create technical debt are **never acceptable**. The time spent designing properly is always less than iterating through wrong approaches. When in doubt, ask: "Am I solving the symptom or the structure?"
- **Config-breaking commits.** If a change would break an existing `foci.toml` (renamed keys, moved sections, changed types — not just new optional fields), the commit message must include an example of the old form and the new form. For example:
	```
	refactor: rename [agent_loop] to [loop]
