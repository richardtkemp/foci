...
func TestHTTPRequestArrayHeaders(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintf(w, "auth=%s", r.Header.Get("Authorization"))
	}))
	defer srv.Close()

	// Use the array-of-objects format that matches the strict schema
	...
	if !strings.Contains(result, "auth=Bearer test-token") {
		t.Errorf("expected header in response: %s", result)
	}
}

func TestHTTPRequestArrayQuery(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintf(w, "q=%s", r.URL.Query().Get("q"))
	}))
	defer srv.Close()

	// Use the array-of-objects format for query params
	params := json.RawMessage(fmt.Sprintf(`{
		"url": %q,
		"query": [
	...
	if !strings.Contains(result, "q=hello world") {
		t.Errorf("expected query param: %s", result)
	}
}

func TestKVPairsUnmarshal(t *testing.T) {
	tests := []struct {
		name  string
		input string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var kv kvPairs
			if err := json.Unmarshal([]byte(tt.input), &kv); err != nil {
